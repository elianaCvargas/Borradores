...
/**
 * \brief Solicita un texto al usuario y  valida que sea alfanumerico
 * \param requestMessage Es el mensaje a ser mostrado para solicitar el dato
 * \param errorMessage Es el mensaje a ser mostrado en caso de error
 * \param lowLimit Es el limite inferior aceptado
 * \param lowLimit Es el limite superior aceptado
 * \param input puntero al lugar donde se cargará el numero ingresado
 * \param attemps indica la cantidad de reintentos ante un error
 * \return 0 si consiguio el texto  -1 si no
 *
 */
...
int getValidAlfaNumerico(char requestMessage[],char errorMessage[], char errorMessageLenght[],char input[], int maxLenght,int attemps)
{
    ...
    for(i=0;i<attemps;i++)
    {
        if (!getAlfaNumerico(requestMessage,buffer))
        {
            printf ("%s",errorMessage);
            continue;
        }
...
/**
 * \brief Solicita un texto alfanumerico al usuario y devuelve el resultado
 * \param mensaje Es el mensaje a ser mostrado
 * \return El caracter ingresado por el usuario
 *
 */
...
/**
 * \brief Verifica si el valor recibido contiene solo letras, números, "@"(arroba) y "." (punto).
 * \param str Array con la cadena a ser analizada
 * \return 1 si contiene solo espacio o letras y números, "@"(arroba) y "." (punto), y 0 si no lo es
 *
 */
...
int esEmail(char str[])
{
   ...
   while(str[i] != '\0')
   {
    ...
    if(contadorGuiones==1 && contador ==1) // debe tener un guion
        return 1;
 ...
 * \param mensaje Es el mensaje a ser mostrado
 * \param input Array donde se cargará el texto ingresado
 * \return 1 si el texto contiene formato de email
 */
int getEmail(char mensaje[],char input[])
{
    char aux[256];
    getString(mensaje,aux);
    if(esEmail(aux))
...
/**
 * \brief Solicita un texto en formato  email al usuario y lo valida
 * \param requestMessage Es el mensaje a ser mostrado para solicitar el dato
 * \param errorMessage Es el mensaje a ser mostrado en caso de error
 * \param lowLimit Es el limite inferior aceptado
 * \param lowLimit Es el limite superior aceptado
 ...
 *
 */
int getValidEmail(char requestMessage[],char errorMessage[], char errorMessageLenght[],char input[], int maxLenght,int attemps)
{
    int i;
    int retorno=-1;
    char buffer[1024];
    ...
    for(i=0;i<attemps;i++)
    {
        if (!getEmail(requestMessage,buffer))
        {
            printf ("%s",errorMessage);
            continue;
        }
